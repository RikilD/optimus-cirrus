...
### 4. Publish the scalac options the internal build uses

`optimus/platform/projects/scala_compat/.../optimus/scala212/DefaultSeq.scala`
exists upstream in both 2.12 and 2.13 variants and supplies `toVarArgsSeq`,
`asScalaUnsafeImmutable` and friends. Nothing in the repo says it is meant to
be injected as a *root import* rather than imported per file, so every
contributor rediscovers this the same way: by adding
`import optimus.scala212.DefaultSeq._` to one file at a time until the build
goes green.

Setting it as a compiler flag on every module that depends on `scala_compat`

```
-Yimports:java.lang,scala,scala.Predef,optimus.scala212.DefaultSeq
```

replaced **16 separate source patches with 5 lines of build config** in our
branch, verified compiling, and removed those 16 files from the rebase surface
entirely. The Gradle attempt worked around the same root cause differently
again (rewriting `asScalaUnsafeImmutable` to `asScala.toSeq` at each call site).

**Ask: publish the scalac option set the internal build applies per scope.**
It is a small, self-contained change and it removes a whole category of
contributor patches rather than one file at a time.
...
### 6. `-Yalias-package`: exported code depends on a private scalac fork

`GraphConsole.scala:107` calls `settings.YaliasPackage.tryToSet(PackageAliases.aliases)`.
`-Yalias-package` is **not a stock scalac option** — verified against
`scala-compiler` 2.13.18, which offers only `-Yimports`, `-Yno-imports` and
`-Yrepl-use-magic-imports`. It is an Optimus addition to the compiler, so this
line cannot compile against a released Scala.

The companion object `optimus.platform.PackageAliases` is **absent from the
export**, so even the alias data is unavailable. `ProcessEnvironment.scala:55`
shows there is a runtime half as well, propagating a `-Doptimus.packageAliases`
JVM argument to child processes.

Our workarounds: comment out the `GraphConsole` call, stub `PackageAliases`
with empty aliases, and reach the setting reflectively in
`RuntimeScalaCompiler` so a stock compiler degrades instead of failing. About
3 files and ~18 lines — small in volume, but it is the one place where exported
code requires a compiler the public cannot obtain.

**Ask, in preference order:**

1. Make the two call sites degrade gracefully on a stock compiler (the
   reflective pattern in our `RuntimeScalaCompiler` works and needs no fork).
2. Publish `PackageAliases`, or state that package aliasing is a no-op outside
   Morgan Stanley.
3. Say plainly in the README that the internal build uses a patched scalac, and
   which flags it adds. Right now nothing signals that, so contributors assume
   a missing build setting and go looking for a flag that does not exist.

### 7. The 24 genuinely-absent classes
...
### 8. `RuntimeScalaCompiler` and `ScalaCompilerConfigurer`
...
subsumes items 3, 9, 10 and most of Tier 3, and it is work that already exists.
